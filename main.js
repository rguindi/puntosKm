const SERVICE_URL = "https://services1.arcgis.com/nCKYwcSONQTkPA4K/ArcGIS/rest/services/Puntos_kilometricos_Espana/FeatureServer/0/query";
...
async function getMunicipioProvincia(lat, lon, maxRetries = 5, delayMs = 500) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const municipio = addr.city || addr.town || addr.village || addr.hamlet || "—";
...
async function buscar(e) {
    ...
    try {
        ...
        if ((!data.features || data.features.length === 0) && /[a-zA-Z]+\d+/.test(carretera)) {
        ...
        data.features.forEach(async (f) => {
            ...
            const gmapsHref = (lat !== "—" && lon !== "—") ? `https://www.google.com/maps?q=${lat},${lon}` : "#";
...
`;
            ...
            if (lat !== "—" && lon !== "—") {
                getMunicipioProvincia(lat, lon).then(mp => {
